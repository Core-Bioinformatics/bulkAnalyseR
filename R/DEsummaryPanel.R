...
#' shiny app. It is generated by including 'Heatmap' in the panels.default argument
    ...
    sidebarLayout(
      ...
      sidebarPanel(
        radioButtons(ns('heatmap.processing'), label = "Heatmap values",
                     choices = c('Expression','Log2 Expression','Z-score'), selected = 'Log2 Expression'),
...
DEsummaryPanelServer <- function(id, expression.matrix, metadata, DEresults, anno){
  ...
  moduleServer(id, function(input, output, session){
    ...
    pca.plot <- reactive({
      ...
      subsetExpression <- expression.matrix()[geneSet,]
      myplot <- plot_pca(
    ...
    heatmap.plot <- eventReactive(input[['goHeatmap']], {
      ...
      if (length(geneSet)==0){geneSet <- anno$NAME[match(head(DEresults()$DE()$DEtableSubset$gene_id,50), anno$ENSEMBL)]}
      geneIDs <- anno$ENSEMBL[match(geneSet, anno$NAME)]
      subsetExpression <- expression.mat[geneIDs,]
