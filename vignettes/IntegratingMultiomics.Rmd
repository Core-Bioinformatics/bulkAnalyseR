...
exp.yang <- read.csv(
  system.file("extdata", "expression_matrix_preprocessed.csv", 
...
```{r}
cis.integration <- tibble::tibble(reference.expression.matrix = 'exp.yang',
...
exp.chip <- read.csv(
  ...
  row.names = 1) %>% as.matrix
meta.chip = data.frame(id = colnames(exp.chip),
                       timepoint = c('0h','12h','36h'))
cis.integration.2 <- tibble::tibble(reference.expression.matrix = c('exp.yang','exp.chip'),
...
generateShinyApp(shiny.dir = 'shiny_Li_2021',
                 ...
                 trans.integration = tibble::tibble(reference.expression.matrix='exp.mrna',
...
head(mirtarbase.comparison.table)
```
...
mirtarbase.comparison.table <- preprocess_miRTarBase(organism.code='mmu',
                                                     org.db='org.Mm.eg.db',
...
```{r, eval=FALSE}
custom.integration <- tibble::tibble(reference.expression.matrix = 'exp.yang',
                                    reference.org.db = 'org.Mm.eg.db',
